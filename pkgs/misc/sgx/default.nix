...
in stdenv.mkDerivation {
    ...
    preBuild = ''
        ...
        tar -zxf ${prebuilt.ae}
        tar -zxf ${prebuilt.dcap} -C external/dcap_source/QuoteGeneration/        
    '';

    # todo: fix up shit in sgx-gdb
    # as it doesn't reference the install loc properly
    installPhase = let build = "build/linux";
    in if type == "sdk" then ''
        # copy headers
        mkdir -p $out/include
        ...
        mkdir -p $out/sdk
        cp common/buildenv.mk $out/sdk/

        ln -s $out/include $out/sdk/include
        ln -s $out/lib $out/sdk/lib

        mkdir -p $out/sdk/bin
        ln -s $out/bin $out/sdk/bin/x86
        ln -s $out/bin $out/sdk/bin/x64
    '' else ''
